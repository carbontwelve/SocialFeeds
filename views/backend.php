<?php
/**
 * @var \Carbontwelve\Widgets\RecentPostsWidget $widget
 * @var string $title
    ...
    <label for="<?php echo $widget->get_field_id( 'feed' ); ?>"><?php echo __('Feed:'); ?>
        <select id="<?php echo $widget->get_field_id( 'feed' ); ?>" name="<?php echo $widget->get_field_name('feed') ?>" class="widefat additionalFieldSelector">
            <?php foreach ($availableFeeds as $feedKey => $feedTitle){ ?>
                <option value="<?php echo $feedKey; ?>" <?php if ($feedKey === $feed){ echo 'selected'; } ?>><?php echo $feedTitle; ?></option>
            <?php } ?>
        </select>
    </label>
</p>
...
<?php
    ...
    ?>
    <div class="additionalFields <?php echo $feedName; ?>" style="<?php echo $style; ?>">
        <?php foreach ($fields as $field ){ ?>
            <p>
                <label for="<?php echo $field['id'] ?>"><?php echo $field['title']; ?>
                    <input type="text" id="<?php echo $field['id']; ?>" name="<?php echo $field['name']; ?>" value="<?php echo esc_attr( $field['value'] ); ?>" class="widefat" />
                </label>
            </p>
        <?php } ?>
    </div>
