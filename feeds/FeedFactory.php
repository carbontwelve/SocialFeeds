...
class FeedFactory
{
    ...
    private function identifyFeeds()
    {
        ...
        $classes = array_filter(scandir($this->lookupPath), function($value){
            if ($value === '.' || $value === '..' || strpos($value, '_') !== false ){ return false; }
            return true;
        });
        ...
        foreach ($classes as $class)
        {
            ...
            /** @var FeedInterface $tmp */
            $tmp = new $className;
            $this->feedScrapers[ $this->slugifyString($tmp->getName()) ] = array(
                'name'        => $tmp->getName(),
